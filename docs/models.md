...
Every provider above works out of the box except xAI: pydantic-ai 2.x uses the
native `xai-sdk`, which cannot be installed together with Sira's dev tools, so
